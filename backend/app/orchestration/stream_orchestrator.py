...
class StreamOrchestrator:
    ...
    async def start(self) -> None:
        if self._worker_task and not self._worker_task.done():
            return
        self._worker_task = asyncio.create_task(self._worker_loop(), name="stream-orchestrator-worker")
        self._fanout_task = asyncio.create_task(self._fanout_loop(), name="event-broadcaster")
    ...
    async def _worker_loop(self) -> None:
        while True:
            _priority, _seq, message = await self.queue.get()
            try:
    ...
    async def _process_message(self, message: ChatMessage) -> None:
        ...
        if reply.should_speak:
            async with self._speech_lock:
                ...
                await self.emit_event("avatar_event", {"event_type": "talking_start", "emotion": reply.emotion})
                await self.emit_event("speaking_status", {"is_speaking": True, "emotion": reply.emotion})
                ...
                await self.emit_event("avatar_event", {"event_type": "talking_stop", "emotion": "idle"})
                await self.emit_event("speaking_status", {"is_speaking": False, "emotion": "idle"})
