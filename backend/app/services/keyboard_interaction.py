...
class KeyboardInteractionService:
        ...
        try:
            ...
            if context is None:
                return AssistantReply(
                    text="I did not press the key because I could not identify a foreground window underneath Sarah.",
                    emotion="concerned",
                    should_speak=True,
                )
    ...
    async def _execute_confirmed_enter(self, user_id: str, pending: PendingEnter) -> AssistantReply:
        ...
        try:
            ...
            if hidden_hwnd is not None:
                await asyncio.sleep(0.25)
            fresh_context = self.context_provider()
            if fresh_context is None or fresh_context.hwnd != pending.hwnd:
                self.pending.cancel(user_id)
                actual = fresh_context.title if fresh_context is not None else "no identifiable foreground window"
                return AssistantReply(
                    text=(
                        f'I did not press Enter because the receiving window changed. It was staged for "{pending.title}", '
                        f'but the current receiver is {actual!r}. Please request Enter again from the current screen.'
                    ),
                    emotion="concerned",
                    should_speak=True,
                )
    ...
    async def handle(self, message: ChatMessage, *, other_system_change_pending: bool = False) -> AssistantReply | None:
        ...
        if request.key == "enter":
            ...
            if context is None:
                return AssistantReply(
                    text="I did not stage Enter because I could not identify which underlying window would receive it.",
                    emotion="concerned",
                    should_speak=True,
                )
            ...
            return AssistantReply(
                text=(
                    ...
                    '2 minutes to re-verify that same receiving window and press Enter once, or "cancel".'
                ),
                emotion="concerned",
                should_speak=True,
            )
